...
v2.3 的图形启动器仍然只是启动器和预览界面，不重写 `file_helper.py` 里的分类、合并、重命名、压缩或撤销规则。GUI 不会绕过 CLI 安全规则，真实整理和真实撤销仍然只能在用户明确确认后，由 GUI 追加一次性 `--yes` 执行。
...
v2.3 启动器使用现代化 CustomTkinter 界面：左侧是模式导航和安全状态，右侧是整理任务工作区。左侧用于切换 `预览模式`、`执行整理`、`撤销上次`，右侧用于填写 Python 命令、脚本路径、root 路径、config 路径，以及查看命令预览、扫描预览表格和运行状态。
