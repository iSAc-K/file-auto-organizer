...
`organizer_run_log.json` 最多保留最近 100 次 apply。写入第 101 次 apply 时，会删除最旧的一整条 apply 记录，包括 `operations` 和历史详情。此规则按记录时间处理所有状态，包括 `running`、`failed`、`partial` 和 `success`，不只删除已完成或成功记录。被删除的记录不再显示在执行历史中，也不能再通过 `--undo-last` 撤销。`rename_log.csv` 不裁剪，继续保留完整的人类审计记录。
