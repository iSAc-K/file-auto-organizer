...
v2.2 的图形启动器仍然只是启动器和预览界面，不重写 `file_helper.py` 里的分类、合并、重命名、压缩或撤销规则。GUI 不会绕过 CLI 安全规则，真实整理和真实撤销仍然只能在用户明确确认后，由 GUI 追加一次性 `--yes` 执行。

新增功能：

- `扫描预览`：读取当前 root 和 `config.yaml`，复用 dry-run 计划生成表格，显示序号、原文件夹、识别日期、识别品类、命中关键词、单量、数量、动作、目标名称、状态和原因。扫描预览不会修改文件，不会重命名，不会移动，不会压缩，不会删除原件，也不会写入真实 undo 操作。
- `打开报告`：尝试打开最近一次生成的 `整理报告.xlsx`。如果当前 root 或 logs 子目录下没有报告，会提示 `未找到整理报告，请先执行扫描预览或整理。`
- 安全状态栏：默认显示 `当前模式：Dry Run｜不会修改文件｜不会压缩｜不会删除原件`。切换到 Apply 会提示需要确认、冲突跳过、不会覆盖已有目标；开启压缩时会提示同名压缩包存在时跳过；撤销模式会提示只根据日志执行、不会猜测路径。
- `预览撤销`：只检查 root 下是否存在 `organizer_run_log.json` 并生成普通撤销命令，不追加 `--yes`，不执行真实撤销。
- `撤销上次整理`：会先弹出确认，说明撤销只根据 `organizer_run_log.json` 执行、不会猜测路径、源路径已存在时跳过；确认后才启动 `--undo-last --yes`。
- 最近目录和配置会自动保存到启动器同目录的 `launcher_settings.json`。该文件只保存路径和 UI 选项，不保存客户文件内容、账号、密码、API Key 或其他敏感信息，并且不会提交到 Git。
...
1. 解压或打开实际发布文件夹，例如 `Windows文件整理助手-v2.2`。
...
v2.2 启动器使用现代化 CustomTkinter 界面：左侧是模式导航和安全状态，右侧是整理任务工作区。左侧用于切换 `预览模式`、`执行整理`、`撤销上次`，右侧用于填写 Python 命令、脚本路径、root 路径、config 路径，以及查看命令预览、扫描预览表格和运行状态。
...
- 点击 `扫描预览` 会在 GUI 表格中显示 dry-run 整理计划，不做真实修改。
- 点击 `打开报告` 会打开最近一次 `整理报告.xlsx`；找不到时会弹窗提示。
- 点击 `预览撤销` 只生成撤销命令并检查日志状态，不执行真实撤销。
- 点击 `撤销上次整理` 会先弹出明确确认，确认后才追加一次性 `--yes` 执行。
