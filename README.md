...
软件启动后会异步检查 GitHub Releases。网络请求临时失败时会自动重试，连续失败会记录到软件目录下的 `update_check.log`。发现新版时由用户选择是否立即更新，不会静默安装。下载包必须通过 SHA-256 校验；整理任务运行期间不能安装更新。更新器保留：

左侧底部的 `检查更新` 按钮会打开独立窗口。检查中、已是最新版、发现新版本、检查失败和下载更新状态都在该窗口内显示；检查失败后可直接点击 `重新检查`。
