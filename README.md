...
v2.5.1 修复了部分打包环境中检查更新失败的问题。发布包会随 EXE 带上 HTTPS 根证书，避免请求 GitHub Releases 时出现 `CERTIFICATE_VERIFY_FAILED`。如果旧版已经无法检查更新，需要先手动下载并解压 v2.5.1，以后的版本再走内置更新。
...
- 旧版本生成的记录如果缺少新增字段，GUI 会提示“详情不完整”。v2.5.2 起，如果旧日志里有明确记录的 `operations`，会额外显示这些操作路径；日期、品类、单量、数量等缺失业务字段仍不会补猜。
